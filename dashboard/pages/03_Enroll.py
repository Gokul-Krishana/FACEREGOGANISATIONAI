...
def _process_enrollment(frame: np.ndarray, emp_id: str, name: str, dept: str | None) -> None:
    ...
    if embedding is None:
        st.error(
            "No face detected in the captured image. Please try again with better lighting and face clearly visible."
        )
        return
    ...
    if matches:
        st.warning(
            f"This face is already enrolled as: **{matches[0]['name']}** (confidence: {matches[0]['confidence']:.1%})"
        )
        return
...
with form_col:
    ...
    with st.form("enroll_form", clear_on_submit=False):
        emp_id = st.text_input(
            "Employee ID *", placeholder="e.g. EMP004", help="Unique identifier, e.g. EMP004"
        )
        ...
        if cam_type == "webcam":
            ...
            if st.session_state["enroll_captured_frame"] is not None:
                ...
                st.image(
                    ...
                    channels="RGB",
                    caption="Captured frame from phone/IP camera",
