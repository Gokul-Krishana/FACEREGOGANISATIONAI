...
from database.models import Attendance, Employee, RecognitionLog, UnknownFace  # noqa: E402
...
def load_attendance_df(days: int = 30, limit: int = 5000) -> pd.DataFrame:
    ...
    try:
        ...
        return pd.DataFrame(
            columns=["date", "hour", "timestamp", "employee_id", "employee_name", "department", "confidence"]
        )
...
def load_recognition_df(days: int = 30, limit: int = 5000) -> pd.DataFrame:
    ...
    try:
        with get_session() as session:
            ...
            return pd.DataFrame(data, columns=["timestamp", "is_known", "confidence", "employee_name"])
    except Exception as _exc:
...
def load_employee_stats() -> pd.DataFrame:
    ...
    try:
        with get_session() as session:
            ...
            return pd.DataFrame(
                data, columns=["id", "employee_id", "name", "department", "total_attendance", "last_seen"]
            )
    except Exception as _exc:
        logger.warning("Could not load attendance-by-employee analytics: %s", _exc)
        return pd.DataFrame(
            columns=["id", "employee_id", "name", "department", "total_attendance", "last_seen"]
        )
    ...
    try:
        with get_session() as session:
            ...
            today = (
                ...
                .filter(UnknownFace.timestamp >= datetime.combine(date.today(), datetime.min.time()))
                .count()
            )
            this_week = session.query(UnknownFace).filter(UnknownFace.timestamp >= cutoff).count()
...
with col4:
    known_count = len(recognition_df[recognition_df["is_known"].eq(True)]) if not recognition_df.empty else 0
    unknown_count = (
        len(recognition_df[recognition_df["is_known"].eq(False)]) if not recognition_df.empty else 0
    )
...
with chart_col3:
    ...
    if not employee_stats.empty:
        ...
        fig_top.update_traces(
            hovertemplate="%{y}<br>%{x} marks<br>Dept: %{customdata[0]}<extra></extra>",
...
with chart_col4:
    ...
    if not recognition_df.empty:
        known = len(recognition_df[recognition_df["is_known"].eq(True)])
        unknown = len(recognition_df[recognition_df["is_known"].eq(False)])
...
with adv_col1:
    ...
    if not attendance_df.empty:
        ...
        heatmap_data["day_name"] = pd.Categorical(
            heatmap_data["day_name"], categories=day_order, ordered=True
        )
...
with st.expander("📋 View Raw Data Tables"):
    ...
    with tab3:
        if not recognition_df.empty:
            ...
            st.dataframe(
                display_rec[["timestamp", "type", "confidence", "employee_name"]],
