...
def fmt(name, vals):
    if not vals: return f'  {name:20s}: no data'
    return (f'  {name:20s}: avg={statistics.mean(vals):7.1f}ms  P50={pct(vals,50):7.1f}ms  '
...
def main():
    ...
    times = {
        'capture': [], 'resize': [], 'full_ai': [],
        'yolo_detect': [], 'face_detect': [], 'embedding': [],
        'faiss_search': [], 'amfr_decide': [],
        'attendance_check': [], 'overlay': [],
    }

    # Wrap detector methods
    orig_detect = service.detector.detect
    orig_crop = service.detector.crop_person
    ...
    orig_amfr = service.amfr._evaluate_person
    ...
    print('='*72)
    print(f'  Duration: {wall:.1f}s | Frames captured: {frame_count} | AI frames: {len(times["full_ai"])} | Skip: {skip}')
